...
public class K8SALSServiceMeshHTTPAnalyzer extends AbstractALSAnalyzer {
    ...
    protected List<ServiceMeshMetric.Builder> analyzeSideCar(final HTTPAccessLogEntry entry) {
        ...
        final LogEntry2MetricsAdapter adapter = newLogEntryAdapter(entry);

        if (cluster.startsWith("inbound|")) {
            // Server side
            final ServiceMeshMetric.Builder metrics = adapter.adaptToDownstreamMetrics()
            ...
            final ServiceMeshMetric.Builder metric = adapter.adaptToDownstreamMetrics()
    ...
    protected List<ServiceMeshMetric.Builder> analyzeProxy(final HTTPAccessLogEntry entry) {
        ...
        final ServiceMeshMetric.Builder outboundMetric = adapter.adaptToUpstreamMetrics()
    ...
    protected LogEntry2MetricsAdapter newLogEntryAdapter(final HTTPAccessLogEntry entry) {
        return new LogEntry2MetricsAdapter(entry);
    }
}
